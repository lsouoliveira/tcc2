...
def plot_result(title, filepath, plt_values, apps, traffics, ylim=None, colors=['b', 'g', 'r', 'c']):
    ...
    for i in range(num_bar):
        app = apps[i]
        plt.bar(index + i * bar_width, plt_values[i], bar_width, color=colors[i], label=app)

    plt.xticks(index + num_bar / 2.0 * bar_width, traffics, fontsize='large')
    plt.ylabel(title, fontsize='x-large')

    if ylim:
        plt.ylim(0, ylim)
        plt.yticks(np.linspace(0, ylim, 11), fontsize='large')
    else:
    ...
    plt.grid(axis='y')
    plt.tight_layout()
    plt.savefig(args.out_dir + ('/%s.png' % (filepath)))
...
"""
    ...
    plt_values = [
        get_value_list_2(throughput, traffics, item, app) for app in apps
    ]
    ...
    plt_values = [
        get_value_list_3(average_delay, traffics, items, app) for app in apps
    ]
    ...
    # 7. Latência bidirecional
    item = 'average_round_trip_delay'

    plt_values = [
        get_value_list_2(average_delay, traffics, item, app) for app in apps
    ]
