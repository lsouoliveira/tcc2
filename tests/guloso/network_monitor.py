...
class NetworkMonitor(app_manager.RyuApp):
    ...
    def route(self, flows):
        ...
        for flow in flows:
            ...
            # Get the source and destination ports of the flow
            src_dp = self.awareness.get_host_location(flow.src)[0]
            dst_dp = self.awareness.get_host_location(flow.dst)[0]

            # Get all the simple paths between src_dp and dst_dp
            paths = self.awareness.shortest_paths.get(src_dp).get(dst_dp)

            # The best path is the one with the highest available bandwidth
            best_path = None
            best_available_bandwidth = None

            # Find the best path
            for path in paths:
                available_bandwidth = self.calc_path_available_bandwidth(path)

                if available_bandwidth is None:
                    continue

                if best_path is None or available_bandwidth > best_available_bandwidth:
                    best_path = path
                    best_available_bandwidth = available_bandwidth
    ...
    def calc_path_available_bandwidth(self, path):
        ...
        available_bandwidth = setting.MAX_CAPACITY + 1
        ...
        for i in range(len(path) - 1):
            if self.awareness.link_to_port.has_key((path[i], path[i + 1])):
                ...
                if self.free_bandwidth.has_key(path[i]) and self.free_bandwidth[path[i]].has_key(src_port):
                    available_bandwidth = min(available_bandwidth, self.free_bandwidth[path[i]][src_port])
