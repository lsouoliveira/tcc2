...
from utils import Benchmark, get_path_by_switch
...
class NetworkMonitor(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Genetico
        self.max_core_switches = int((CONF.fanout / 2) ** 2)
        self.max_agg_switches = int(CONF.fanout * CONF.fanout / 2)

        self.num_agg_switchs_by_pod = int(self.max_agg_switches / CONF.fanout)

        self.core_switches = [self.format_datapath(1, n) for n in range(1, self.max_core_switches + 1)]
        self.agg_switches = [] 
        self.edge_switches = [] 
        self.edge_switch_map = None 

        for agg_switch_index in range(1, self.max_agg_switches + 1):
            if (agg_switch_index - 1) % self.num_agg_switchs_by_pod == 0:
                self.agg_switches.append([])

            self.agg_switches[-1].append(
                    self.format_datapath(2, agg_switch_index)
                    )

            self.edge_switches.append(
                    self.format_datapath(3, agg_switch_index)
                    )

    def create_edge_switch_map(self):
        edge_switch_map = {}

        for src in self.edge_switches:
            for dst in self.edge_switches:
                if not edge_switch_map.has_key(src):
                    edge_switch_map[src] = {}

                if not edge_switch_map[src].has_key(dst):
                    edge_switch_map[src][dst] = {}

                is_interpod = self.is_interpod(src, dst) 

                if is_interpod:
                    for switch in self.core_switches:
                        edge_switch_map[src][dst][switch] = get_path_by_switch(self.graph, src, dst, switch)
                else:
                    for switch in self.agg_switches[self.get_pod_index(src)]:
                        edge_switch_map[src][dst][switch] = get_path_by_switch(self.graph, src, dst, switch)
    ...
    def _save_bw_graph(self):
        ...
        while CONF.weight == 'bw' or CONF.weight == 'hop':
            ...
            if not nx.is_empty(self.graph) and self.edge_switch_map is None:
                self.create_edge_switch_map()
        ...
        for stat in sorted([flow for flow in body if ((flow.priority not in [0, 65535]) and (flow.match.get('ipv4_src')) and (flow.match.get('ipv4_dst')))],
            ...
            if str(dpid).startswith('3'):
                ...
                self.flows.append({'src': src, 'dst': dst, 'demand': flowDemand,
                    'size': (speed * 8) / 1000, 'converged':False, 'receiver_limited': False,
                    'match': stat.match, 'priority': stat.priority, 'timestamp': time.time()})
            else:
                pass

        # Estimate flows' demands if all the flow_stat replies are received.
        if len(self.statRecord) == 1.25 * (CONF.fanout ** 2) and self.flows:
            flows = [network_models.from_dict(flow) for flow in self.flows]

            self.route(flows)
        else:
            pass

    def route(self, flows):
        if nx.is_empty(self.graph) or self.edge_switch_map is None:
            return

        elephant_flows = [flow for flow in flows if flow.demand > 0.1] 
        ...
        for flow in elephant_flows:
            ...
            is_interpod = self.is_interpod(src_dp, dst_dp)

            demands.append({
                'src': src_dp,
                'dst': dst_dp,
                'size': flow.size,
                'interpod': is_interpod,
                'pod': self.get_pod_index(src_dp) if not is_interpod else None
                })
        ...
        ga = NetworkGa(
                ...
                agg_switches=self.agg_switches,
                fattree=self.graph,
                max_capacity=100000,
                population_size=50,
                generations=100,
        ...
        ga.bm.end().print_results()

        _, results = ga.best_individual()

        for demand_index in range(len(demands)):
            path = get_path_by_switch(self.graph, demands[demand_index]['src'], demands[demand_index]['dst'], results[demand_index])
            self.custom_install_flow(elephant_flows[demand_index], path)

    def format_datapath(self, switch_level, index):
        return int('%d%03d' % (switch_level, index))

    def is_interpod(self, src_dp, dst_dp):
        src_dp_pod_index = int((int(str(src_dp)[1:]) - 1) / self.num_agg_switchs_by_pod)
        dst_dp_pod_index = int((int(str(dst_dp)[1:]) - 1) / self.num_agg_switchs_by_pod)

        return src_dp_pod_index != dst_dp_pod_index

    def get_pod_index(self, src_dp):
        return int((int(str(src_dp)[1:]) - 1) / self.num_agg_switchs_by_pod)

    def custom_install_flow(self, flow, path):
        match = flow.match
