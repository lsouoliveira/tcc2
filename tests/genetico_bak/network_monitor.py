# Copyright (C) 016 Huang MaChi at Chongqing University
...
class NetworkMonitor(app_manager.RyuApp):
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        if len(self.statRecord) == 1.25 * (CONF.fanout ** 2) and self.flows:
            ...
            # Trigger FlowsDetected event
            self.route(flows)

    def route(self, flows):
        '''Route flows by choosing the path with the highest available bandwidth

        Args:
            flow (Flow): the flow that will be routed
        '''

        #if not any([flow.demand > 0.1 for flow in flows]):
        #    return
        ...
        ga.total_time_bm.end().print_results()
