...
class NetworkGa(GeneticAlgorithm):
    def __init__(self,
        ...
        self.total_time_bm = Benchmark('TOTAL_TIME').start()
    ...
    def selection(self, population):        
        ...
        p_fitness_list = map(lambda x: float(x.fitness) / fitness_sum, population)

        # Select individual by probability p
        choice = np.random.choice(population, len(population), p=p_fitness_list)
            ...
            for l in range(1, len(path)):
                ...
                if links_map.has_key(path[l]) and links_map[path[l]].has_key(path[l - 1]):
                    ...
                    if links_map.has_key(path[l]):
                        a = path[l]
                        links_map[a][path[l - 1]] = link['bandwidth']
                        b = path[l - 1]
                    elif links_map.has_key(path[l - 1]):
                        a = path[l - 1]
                        links_map[a][path[l]] = link['bandwidth']
                        b = path[l]
                    else:
                        links_map[path[l]] = {}
                        a = path[l]
                ...
                new_link_utilization = float(links_map[a][b]) / self.max_capacity

                max_link_utilization = max(new_link_utilization, max_link_utilization)

        return max_link_utilization
